...
struct CoreMemberTest {
    ...
    @Test func coreMember_파트선택() async throws {
        let expectation = XCTestExpectation(description: "유저의  파트선택")
        let selectPart: SelectPart = .iOS
        await testStore.send(.appearSelectPart(selectPart: selectPart))
    }
    
    @Test func coreMember_날짜선택() async throws {
        let expectation = XCTestExpectation(description: "날짜 필터 선택")
    ...
    @Test func fetchCoreMember_리스트테스트() async throws {
        ...
        await testStore.send(.fetchMember)
    }
    ...
    @Test func attendaceModel_업데이트테스트() async throws {
        ...
        await testStore.send(.fetchCurrentUser)
        ...
        await testStore.send(.updateAttendanceModel([updateModel]))
    ...
    @Test func test_실시간데이터로드() async throws {
        ...
        await testStore.send(.observeAttendance)
    }
